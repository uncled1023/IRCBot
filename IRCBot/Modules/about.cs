                                ...
                                switch (trigger)
                                {
                                    ...
                                    case "runtime":
                                        ...
                                        if (nick_access >= command_access)
                                        {
                                            DateTime now = new DateTime();
                                            now = DateTime.Now;
                                            int days = now.Subtract(ircbot.ircbot.run_time).Days;
                                            int hours = now.Subtract(ircbot.ircbot.run_time).Hours;
                                            int minutes = now.Subtract(ircbot.ircbot.run_time).Minutes;
                                            int seconds = now.Subtract(ircbot.ircbot.run_time).Seconds;
                                            string uptime = "";
                                            if (days > 0)
                                            {
                                                uptime += days + " days, ";
